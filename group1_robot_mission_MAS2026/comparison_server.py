...
SCENARIOS = [
    {
        ...
        "no_comms": True,
        "kwargs": dict(
            green_robot_is_random=True,  green_robot_has_memory=False,
            yellow_robot_is_random=True, yellow_robot_has_memory=False,
            red_robot_is_random=True,    red_robot_has_memory=False,
        ),
    },
    {
        "label": "Scénario 2",
        ...
        "no_comms": True,
        "kwargs": dict(
            green_robot_is_random=False,  green_robot_has_memory=True,
            yellow_robot_is_random=False, yellow_robot_has_memory=True,
            red_robot_is_random=False,    red_robot_has_memory=True,
        ),
    },
    {
        "label": "Scénario 3",
...
DEFAULT_PARAMS = dict(
    width=18, height=14,
    n_green_robots=4, n_yellow_robots=3, n_red_robots=2,
    n_green_wastes=20, max_step=200,
)
        ...
        if scenario["no_comms"]:
            for agent in m.agents:
                if hasattr(agent, "_broadcast_knowledge"):
                    agent._broadcast_knowledge = types.MethodType(_noop_broadcast, agent)
        result.append(m)
    return result
...
def _draw_grid(ax, model, accent):
    ...
    ax.add_patch(patches.Rectangle((zw,0), zw, H, color=ZONE_COLORS[Zone.Z2], zorder=0))
    ax.add_patch(patches.Rectangle((2*zw,0), W-2*zw, H, color=ZONE_COLORS[Zone.Z3], zorder=0))
    ax.add_patch(patches.Rectangle((0,0), W, H, fill=False, edgecolor="#555", linewidth=1.5, zorder=1))

    for x in range(W+1): ax.axvline(x, color="gray", linewidth=0.2, alpha=0.35)
    for y in range(H+1): ax.axhline(y, color="gray", linewidth=0.2, alpha=0.35)
    ...
    for x in range(W):
        for y in range(H):
            ...
            if any(isinstance(a, WasteDisposalZone) for a in cell):
                ax.add_patch(patches.Rectangle((x+0.05,y+0.05), 0.9, 0.9,
                                               color=DISPOSAL_COLOR, alpha=0.3, zorder=1))
                ax.text(x+0.5, y+0.5, "⬛", ha="center", va="center", fontsize=7, zorder=3)
            wo = 0.0
            for a in cell:
                if isinstance(a, Waste) and a.pos is not None:
                    ax.add_patch(patches.Rectangle((x+0.1+wo, y+0.1), 0.22, 0.22,
                                                   color=WASTE_COLORS[a.waste_type], zorder=2))
                    wo += 0.20
            for a in cell:
                for RCls, rcol in ROBOT_COLORS.items():
                    if isinstance(a, RCls):
                        ax.add_patch(plt.Circle((x+0.5,y+0.5), 0.33, color=rcol, zorder=4))
                        short = {GreenRobot:"G", YellowRobot:"Y", RedRobot:"R"}[RCls]
                        tc = "white" if RCls != YellowRobot else "#333"
                        ax.text(x+0.5,y+0.5, f"{len(a.carrying)}{short}",
    ...
    ax.set_title(f"{status}  —  déposés : {model.nb_collected_wastes}",
                 fontsize=9, color=accent, pad=16)


def _draw_ratio(ax, model, accent):
    df = model.datacollector.get_model_vars_dataframe()
    if df.empty or "Ratio collecté" not in df.columns:
        ax.text(0.5, 0.5, "Pas encore de données", ha="center", va="center",
    ...
    ax.set_xlim(0, model.max_step); ax.set_ylim(0,1)
    ax.set_xlabel("Step", fontsize=8); ax.set_ylabel("Ratio collecté", fontsize=8)
    ax.tick_params(labelsize=7); ax.grid(True, alpha=0.2)
    if len(df) > 0:
        final = df["Ratio collecté"].iloc[-1]
        ax.annotate(f"{final:.0%}", xy=(df.index[-1], final),
...
def ScenarioPanel(model, meta, tick):
    ...
    with solara.Column(style={
        "border": f"2px solid {accent}", "border-radius": "10px",
        "padding": "12px", "background": "#fafafa",
        "min-width": "300px", "flex": "1",
    }):
        solara.Text(meta["label"],
                    style={"font-weight": "700", "font-size": "15px", "color": accent})
        solara.Text(meta["subtitle"],
...
def ComparisonView():
    ...
    n_green_robots,  set_n_green_robots  = solara.use_state(DEFAULT_PARAMS["n_green_robots"])
    n_yellow_robots, set_n_yellow_robots = solara.use_state(DEFAULT_PARAMS["n_yellow_robots"])
    n_red_robots,    set_n_red_robots    = solara.use_state(DEFAULT_PARAMS["n_red_robots"])
    n_green_wastes,  set_n_green_wastes  = solara.use_state(DEFAULT_PARAMS["n_green_wastes"])
    ...
    def build_params():
        return dict(width=width, height=height,
                    n_green_robots=n_green_robots, n_yellow_robots=n_yellow_robots,
                    n_red_robots=n_red_robots, n_green_wastes=n_green_wastes,
                    max_step=max_step)
    ...
    # ── UI ────────────────────────────────────────────────────────────────────
    with solara.Column(style={"padding": "20px", "font-family": "sans-serif", "max-width": "1600px"}):
        solara.Text("Comparaison de scénarios",
                    style={"font-size": "22px", "font-weight": "800", "margin-bottom": "2px"})
        solara.Text("Même grille initiale — trois stratégies robots",
                    style={"font-size": "13px", "color": "#666", "margin-bottom": "16px"})

        with solara.Card(title="Paramètres", elevation=1, style={"margin-bottom": "14px"}):
            with solara.Row(style={"flex-wrap": "wrap", "gap": "24px"}):
                with solara.Column(style={"min-width": "200px"}):
                    solara.SliderInt("Graine",  value=seed,   on_value=set_seed,   min=0,  max=999)
                    solara.SliderInt("Largeur", value=width,  on_value=set_width,  min=6,  max=36, step=3)
                    solara.SliderInt("Hauteur", value=height, on_value=set_height, min=3,  max=36)
                with solara.Column(style={"min-width": "200px"}):
                    solara.SliderInt("Robots verts",  value=n_green_robots,  on_value=set_n_green_robots,  min=1, max=10)
                    solara.SliderInt("Robots jaunes", value=n_yellow_robots, on_value=set_n_yellow_robots, min=1, max=10)
                    solara.SliderInt("Robots rouges", value=n_red_robots,    on_value=set_n_red_robots,    min=1, max=10)
                with solara.Column(style={"min-width": "200px"}):
                    solara.SliderInt("Déchets initiaux", value=n_green_wastes, on_value=set_n_green_wastes, min=4, max=64, step=4)
                    solara.SliderInt("Pas maximum",      value=max_step,       on_value=set_max_step,       min=50, max=500, step=10)

        with solara.Row(style={"gap": "10px", "margin-bottom": "18px", "align-items": "center"}):
            ...
            if models:
                solara.Text(f"Step : {models[0].steps}",
        ...
        if not models:
            ...
            with solara.Row(style={"gap": "14px", "align-items": "flex-start", "flex-wrap": "wrap"}):
